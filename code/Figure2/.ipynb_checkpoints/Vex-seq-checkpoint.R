...
#merged[Method=='HAL' & side!='']$Prediction = 0
...
correlation <- merged[, cor(Prediction, HepG2_delta_psi), 
...
##' ## only capable points
MMSplice_cor <- merged[Method=='MMSplice'][, cor(Prediction, HepG2_delta_psi)]
hal_cor <- merged[side==''][(isSNP)][Method=='HAL'][, cor(Prediction, HepG2_delta_psi)]
spanr_cor <- merged[!(SPANR_NA)][Method=='SPANR'][, cor(Prediction, HepG2_delta_psi)]
